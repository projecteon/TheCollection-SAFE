    ...
    Pagination.pagination [ Pagination.IsCentered; Pagination.CustomClass "is-sticky-top" ]
      [
        Pagination.previous [ Props [ Disabled (model.currentPage < 2); OnClick (fun ev -> dispatch SearchPagePrevious) ] ]
          [ str "Previous" ]
        Pagination.next [ Props [Disabled (model.currentPage = numberOfPages); OnClick (fun ev -> dispatch SearchPageNext)] ]
          [ str "Next page" ]
        Pagination.list [ ]
          [
            model|> getPageList numberOfPages |> createPageButtons model.currentPage dispatch |> ofList
          ]
      ]
